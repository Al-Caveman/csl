...
\title{
3.1.0 \<طَبَقَةُ مِقْبَسِ رَجُلِ الكهفِ (طَمِرَك) إصدارة>\\
...
All CSL data are communicated using messages that are formated as per the TLV
tuple \begin{tabular}{|l|l|l|}\hline
$t$ (8 bits) & $n$ (32 bits) & $d$ (length bits)\\\hline
\end{tabular}, with large-endian encoding.

\section{Message Types}
\subsection{Initial Encryption Channel}
\begin{itemize}
    \item $t = 0$.
    \item $n \in \{1,2,\ldots,2^{32}\}$.
    \item $d \in \{0,1,\ldots,8\}^n$.
    \item Usage: used to signal that the sending peer is interested in finding
    a shared key based on $d$ and DH, and then to start communicating over a
    symmetric encrypted channel that uses the shared key. 
    \item Behaviour:
        \begin{enumerate}
